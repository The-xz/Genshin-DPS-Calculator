...
async function main() {
  const url = `https://api.lunaris.moe/data/${API_VERSION}/${LANGUAGE}/char/${CHARACTER_ID}.json`;
  ...
  if (!level90Stats) {
    throw new Error("Could not find level 90 stats");
  }
